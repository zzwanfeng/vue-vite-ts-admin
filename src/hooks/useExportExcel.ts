...
export function exportJsonToExcel(params: jsonTypes) {
	...
	if (autoWidth) {
		...
		const colWidth = data.map((row: ({ toString: () => { (): any; new(): any; charCodeAt: { (arg0: number): number; new(): any }; length: number } } | null)[]) =>
			// eslint-disable-next-line no-unused-vars
			row.map((val: { toString: () => { (): any; new(): any; charCodeAt: { (arg0: number): number; new(): any }; length: number } } | null) => {
